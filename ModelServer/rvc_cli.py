...
def load_voices_data():
    tts_voices_path = os.path.join(current_script_directory, "rvc", "lib", "tools", "tts_voices.json")
    with open(tts_voices_path, "r", encoding="utf-8") as f:
        return json.load(f)


voices_data = load_voices_data()
locales = [voice["ShortName"] for voice in voices_data] # ShortName 사용
...
def parse_arguments():
    ...
    tts_parser.add_argument(
        "--tts_file", type=str, help="File with a text to be synthesized", required=False
    )
    tts_parser.add_argument(
    ...
    try:
        if args.mode == "infer":
            ...
            run_tts_script(
                ...
                #input_path=args.input_path,
                #output_path=args.output_path,
                output_tts_path=args.output_tts_path, # 정상 작동
